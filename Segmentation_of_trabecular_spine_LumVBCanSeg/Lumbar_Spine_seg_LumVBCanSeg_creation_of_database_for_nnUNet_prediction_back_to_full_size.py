...
if __name__ == "__main__":
    ...
    crop_parameters_folder = join(path_to_predicted_segmentations, "Predict_clin_data_all_crop_rotovane_zpet_full_size") 
    ...
    for t in predict_pacients: 
        ...
        #load predicted data
        predicted_file=path_to_predicted_segmentations+'/Predict_clin_data_all_crop_rotovane_zpet/'+t+'.nii.gz'
        ...
        # output folder for full size image
        database_folder = join(base,t,'Spine_labels','Trabecular_masks') 
        maybe_mkdir_p(database_folder) 
        nib.save(pom_full_size_mask, join(database_folder, t + version_name + ".nii.gz"))
 
    '''
    # verze přímo s nahráním do databáze
    base = 'E:\Analysis_of_MM_patients/'   
    task_id = 908
    version_name = "_trabecular_seg_nnUNet_v_1_0"
    task_name = "Lumbar_spine_seg" 
    foldername = "Dataset%03.0d_%s" % (task_id, task_name) 
    ...
    predict_pacients=['Healthy_006','Healthy_007','Healthy_008','Healthy_009','Healthy_010','Myel_001', 'Myel_002', 'Myel_003', 'Myel_004', 'Myel_005']
    for t in predict_pacients: 
        current_path=join(path_to_crop_parameters,t)
        #open json file and get coordinates
        f=open(current_path+'.json')
        ...
        # output folder for full size image
        crop_parameters_folder = join(base,t,'Spine_labels','Trabecular_masks') 
        maybe_mkdir_p(crop_parameters_folder) 
